...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      const gen = ++this._gen;
      try {
        // Floor of 480 so a slot that happens to be small (or narrow at drop
        // time, e.g. a mobile viewport) still captures a retina-sharp image —
        // without this, toDataUrl's targetW*2 cap could lock in a blurry
        // encode permanently, since the original file is discarded after ingest.
        const w = Math.max(this.clientWidth || this.offsetWidth || MAX_DIM, 480);
